...
public class NaivePyramidSolverTest {
    ...
    public void solverHandlesSampleData() {
        ...
        assertEquals("Naive:Max path in Sample pyramid", 24, solver.pyramidMaximumTotal(pyramid));
    }

    @Test
    public void solverHandlesDemoData() {
        Pyramid pyramid = new Pyramid(DEMO_DATA);
        assertEquals("Naive:Max path in Demo pyramid", 353, solver.pyramidMaximumTotal(pyramid));
    }

    /* this algorithm leads to 2^100 calculations, we should pass it */
    public void solverSurvivesLargeData() {
        ...
        assertTrue("Naive:Max path in a large pyramid not positive", solver.pyramidMaximumTotal(pyramid) > 0L);
    }
    ...
    public void solverHandlesRandomData() {
        ...
        assertEquals("Naive:Max path in 'random' pyramid", 398, this.solver.pyramidMaximumTotal(pyramid));
    }
}
